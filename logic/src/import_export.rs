...
mod tests {
    ...
    #[test]
    fn export_json_wrong_extension() {
        let dir = tempdir().unwrap();
        let file_path = dir.path().join("test.txt");

        let result = create_export_json(&file_path);
        assert!(matches!(result, Err(ImportExportError::NotJson)));
    }

    #[test]
    fn export_json_invalid_file_path() {
        let dir = tempdir().unwrap();
        let file_path = dir.path().join("test");
    ...
    #[test]
    fn export_json_valid_file_path() {
        let dir = tempdir().unwrap();
        let file_path = dir.path().join("test.json");
    ...
    #[test]
    fn import_json_invalid_file_path() {
        let dir = tempdir().unwrap();
        let file_path = dir.path().join("test");
    ...
    #[test]
    fn import_json_file_does_not_exist() {
        let dir = tempdir().unwrap();
        let file_path = dir.path().join("test.json");
